...
func (q *Queries) JobApplyWorkflowWait(ctx context.Context, db DBTX, arg *JobApplyWorkflowWaitParams) (*RiverJob, error) {
	row := db.QueryRow(ctx, jobApplyWorkflowWait, arg.Outcome, arg.Now, arg.ID)
	var i RiverJob
	err := row.Scan(
		&i.ID,
		&i.Args,
...
func (q *Queries) JobCancelWorkflow(ctx context.Context, db DBTX, arg *JobCancelWorkflowParams) ([]*RiverJob, error) {
	rows, err := db.Query(ctx, jobCancelWorkflow,
		arg.Now,
		arg.CancelAttemptedAt,
		arg.Reason,
		arg.WorkflowID,
...
func (q *Queries) JobGetWorkflowDeadlineExpired(ctx context.Context, db DBTX, arg *JobGetWorkflowDeadlineExpiredParams) ([]*RiverJob, error) {
	rows, err := db.Query(ctx, jobGetWorkflowDeadlineExpired, arg.Now, arg.AfterID, arg.Max)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT id, args, attempt, attempted_at, attempted_by, created_at, errors, finalized_at, kind, max_attempts, metadata, priority, queue, state, scheduled_at, tags, unique_key, unique_states
FROM /* TEMPLATE: schema */river_job
WHERE metadata->>'river:workflow_id' = $1::text
  AND (
    cardinality($2::text[]) = 0
    OR metadata->>'river:workflow_task' = ANY($2::text[])
  )
ORDER BY id
`
...
func (q *Queries) JobGetWorkflowTasks(ctx context.Context, db DBTX, arg *JobGetWorkflowTasksParams) ([]*RiverJob, error) {
	rows, err := db.Query(ctx, jobGetWorkflowTasks, arg.WorkflowID, arg.TaskNames)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) JobGetWorkflowWaitTasks(ctx context.Context, db DBTX, arg *JobGetWorkflowWaitTasksParams) ([]*RiverJob, error) {
	rows, err := db.Query(ctx, jobGetWorkflowWaitTasks, arg.AfterID, arg.Max)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) JobRetryWorkflow(ctx context.Context, db DBTX, arg *JobRetryWorkflowParams) ([]*RiverJob, error) {
	rows, err := db.Query(ctx, jobRetryWorkflow, arg.ResetHistory, arg.WorkflowID, arg.TargetStates)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const jobUpdateWorkflowReady = `-- name: JobUpdateWorkflowReady :many
WITH candidates AS (
...
func (q *Queries) JobUpdateWorkflowReady(ctx context.Context, db DBTX, arg *JobUpdateWorkflowReadyParams) ([]*RiverJob, error) {
	rows, err := db.Query(ctx, jobUpdateWorkflowReady, arg.Now, arg.Max)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
